...
public interface HoldingRelationRepository extends Neo4jRepository<HoldingRelation,Long> {
    @Query("MATCH (e1:Company)-[r:持股*1..4]->(e2:Company) where ID(e1)={0} and (ID(e1)<ID(e2) or ID(e1)>ID(e2)) return e1,e2,r")
    Set<HoldingRelation> getMultiDepthChildrenCompaniesRelations(long id);

    @Query("MATCH p=()-[r:持股]->(e2:Company) where ID(e2)={0} and r.isLargestHolding='1' return p")
    HoldingRelation getBiggestHolder(long id);

    @Query("MATCH (e1:Company)-[r:持股*1..6]->(e2:Company) where ID(e2)={0} and (ID(e1)<ID(e2) or ID(e1)>ID(e2)) return e1,e2,r")
